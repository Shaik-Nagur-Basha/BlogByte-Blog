...
`BlogByte Blog` is a full-stack blogging platform built using the `MERN (MongoDB, Express, React, Node.js) stack`. This platform allows users to create, view, and manage blog posts with user authentication and real-time updates. It utilises `RESTful APIs` for backend operations and follows the `MVC (Model-View-Controller)` design pattern for better scalability and organisation.

---

#### **What's New in Version 3**

1. **Cloud Image Uploads via Cloudinary**
   - Replaced **Firebase Cloud Storage** with **Cloudinary** for all image uploads (profile pictures, post cover images).
   - Images are processed server-side using `multer` + `multer-storage-cloudinary` and stored under the `BlogByte Blog` folder in Cloudinary.
   - No client-side Firebase SDK dependency for storage — uploads now go through a dedicated `POST /api/upload/image` endpoint.

2. **New Upload API Route**
   - Added `api/routes/uploadRoute.js` — a protected route that accepts a multipart image and returns the Cloudinary URL.
   - Added `api/utils/cloudinary.js` — centralised Cloudinary configuration and storage adapter.
...
2. **Enhanced User Experience**
   ...
   - **Improved Responsiveness**: Optimised design for compatibility across all devices, from mobile to desktop.
...
Here are some screenshots of `Admin Pages`: [Click here](#screenshots)

---

## Features

- `Admin Role`: Manage user accounts and admin role approvals.
...
- `Cloudinary Image Uploads`: Profile pictures and post cover images are uploaded and served via Cloudinary CDN.
...
- `Rich Text Editor`: Provides a rich text editor (jodit-react) for writing blog posts.
...
- `Multer` + `multer-storage-cloudinary`: Middleware for handling multipart image uploads directly to Cloudinary.

### Frontend:
- `React.js`: JavaScript library for building user interfaces.
- `Redux`: State management for consistent data flow in the app.
- `Tailwind CSS` / `Flowbite React`: For responsive and modern design.
- `Firebase` (Auth only): Google OAuth sign-in via Firebase Authentication.

### API:
- `RESTful APIs`: Handling CRUD operations for blogs, users, comments, likes, and image uploads.
...
- Firebase project (for Google OAuth only).
...
3. **Set up environment variables**:
   ...
   ```env
   MONGODB_URL=<mongodb+srv://<username>:<password>@cluster0.xxxxx.mongodb.net/<database-name>?retryWrites=true&w=majority&appName=Cluster0>
   JWT_SECRET=<Random String>
   ADMIN_ACCOUNT_ID=<MongoDB _id of the admin user>
   ...
   ```env
   # Firebase — https://console.firebase.google.com (used for Google OAuth only)
...
├── api/                            # Backend code (Node.js, Express.js, MongoDB & JWT)
├── api/controllers/                # API controllers
├── api/models/                     # Database models
├── api/routes/                     # API routes
│   ├── authRoute.js                # Authentication routes
│   ├── userRoute.js                # User management routes
│   ├── postRoute.js                # Blog post routes
│   ├── commentRoute.js             # Comment routes
│   └── uploadRoute.js              # Cloudinary image upload route
├── api/utils/                      # Utility functions
│   ├── cloudinary.js               # Cloudinary config & multer storage adapter
